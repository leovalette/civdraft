...
export default function LobbyPage({
  ...
  const isObserver = useMemo(() => {
    ...
    return lobby.observers.some((player) => player.id === userId);
  }, [lobby, userId]);
  ...
  return (
    <div className="flex h-screen w-10/12 flex-col text-white">
      ...
      <div className="mt-6 flex justify-center p-4">
        <CivPrimaryButton disabled={isObserver} onClick={handleToggleReady}>
          Ready
        </CivPrimaryButton>
      </div>
      <div className="mt-6 flex justify-center">
        {lobby?.autoBannedLeaderIds && lobby.autoBannedLeaderIds.length > 0 && (
