...
export const usePromoteToAdmin = () => {
  return useMutation(api.admin.promoteToAdmin);
};

/**
 * Hook to demote an admin to user
 */
export const useDemoteToUser = () => {
  return useMutation(api.admin.demoteToUser);
};

/**
 * Hook to list all users (only works for admins)
 */
export const useListAllUsers = () => {
  return useQuery(api.admin.listAllUsers);
};
