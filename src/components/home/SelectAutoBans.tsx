...
type Props = {
    bannableLeaders: { name: string; src: string }[]
    bansCiv: { name: string; id: string; src: string }[]
    setBansCiv: (bansCiv: { name: string; src: string }[]) => void
}
export const SelectAutobans: FC<Props> = ({ bansCiv, setBansCiv, bannableLeaders }) => (
    <div>
        ...
        <Select
            ...
            value={bansCiv}
            onChange={(leader: MultiValue<{ name: string; src: string }>) => {
                setBansCiv(leader as { name: string; src: string }[])
            }}
            isSearchable={true}
