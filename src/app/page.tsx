...
export default function Home() {
  ...
  const bannableLeaders = useMemo<{ name: string; src: string }[]>(() =>
    leaders?.filter(({ name }) => name !== "TIMEOUT").map(({ name, imageName }) => ({ name, src: imageName })) ?? []
    , [leaders])

  const onPresetChange = useCallback((e: { presetId: string; label: string | null } | null) => {
    setPreset(
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      const lobbyId = await createLobby({
        ...
        numberOfPicksSecondRotation: selectedPreset?.numberOfPicksSecondRotation,
  ...
  return (
    <div className="w-96 flex flex-col gap-4 rounded-lg border border-golden-border bg-gradient-to-br from-indigo-200 p-6 text-bg-golden-border">
      ...
      />
      <SelectAutobans bansCiv={banCivs} setBansCiv={setBanCivs} bannableLeaders={bannableLeaders} />
      <SelectMaps
