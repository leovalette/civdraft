...
export const startMapDraft = mutation({
  ...
  handler: async (ctx, { lobbyId }) => {
    ...
    await ctx.scheduler.runAfter(MAP_BAN_TIMEOUT_MS, internal.mapDraft.checkMapBanTimeout, {
      lobbyId,
...
export const checkMapBanTimeout = internalMutation({
  ...
  handler: async (ctx, { lobbyId, timestampAtStart }) => {
    ...
    await performMapBan(ctx, lobbyId, DEFAULT_AUTO_BAN_MAP_ID, lobby.currentTeamTurn || 1)
  },
})
